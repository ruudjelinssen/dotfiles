...
function send_notification {
	...
	$DIR/notify-send.sh "" -i "$icon_name" -t 2000 -h int:value:"$volume" -h string:synchronous:"─" --replace=555
		else
...
case $1 in
    ...
    mute)
		...
		if is_mute ; then
		DIR=`dirname "$0"`
		$DIR/notify-send.sh -i "notification-audio-volume-muted" --replace=555 -u normal "Mute" -t 2000
		else
			send_notification
		fi
		;;
